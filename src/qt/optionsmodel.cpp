...
void OptionsModel::Init()
{
    ...
    if (!settings.contains("nDisplayUnit"))
        settings.setValue("nDisplayUnit", BitcoinUnits::MTNS);
    ...
    if (!settings.contains("nAnonymizeCHIPOAmount"))
        settings.setValue("nAnonymizeCHIPOAmount", 1000);

    nAnonymizeCHIPOAmount = settings.value("nAnonymizeCHIPOAmount").toLongLong();
...
// Wallet
    ...
    if (settings.contains("nAnonymizeCHIPOAmount"))
        SoftSetArg("-anonymizechipoamount", settings.value("nAnonymizeCHIPOAmount").toString().toStdString());
...
void OptionsModel::Reset()
{
    ...
    settings.clear();
    resetSettings = true; // Needed in chipo.cpp during shotdown to also remove the window positions
...
QVariant OptionsModel::data(const QModelIndex& index, int role) const
{
    if (role == Qt::EditRole) {
        ...
        case AnonymizeCHIPOAmount:
            return QVariant(nAnonymizeCHIPOAmount);
        case Listen:
...
bool OptionsModel::setData(const QModelIndex& index, const QVariant& value, int role)
{
    ...
    if (role == Qt::EditRole) {
        ...
        case AnonymizeCHIPOAmount:
            nAnonymizeCHIPOAmount = value.toInt();
            settings.setValue("nAnonymizeCHIPOAmount", nAnonymizeCHIPOAmount);
            emit anonymizeCHIPOAmountChanged(nAnonymizeCHIPOAmount);
            break;
        case CoinControlFeatures:
