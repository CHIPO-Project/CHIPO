                                                          ...
                                                          currentBalance(-1)
{
    ...
    // "Spending 999999 zMTNS ought to be enough for anybody." - Bill Gates, 2017
    ui->zMTNSpayAmount->setValidator( new QDoubleValidator(0.0, 21000000.0, 20, this) );
    ...
    ui->labelzMTNSSyncStatus->setText("(" + tr("out of sync") + ")");
    ...
    if(GetAdjustedTime() > GetSporkValue(SPORK_16_ZEROCOIN_MAINTENANCE_MODE)) {
        ui->pushButtonMintzMTNS->setEnabled(false);
        ui->pushButtonMintzMTNS->setToolTip(tr("zMTNS is currently disabled due to maintenance."));

        ui->pushButtonSpendzMTNS->setEnabled(false);
        ui->pushButtonSpendzMTNS->setToolTip(tr("zMTNS is currently disabled due to maintenance."));
    }
}
...
void PrivacyDialog::on_addressBookButton_clicked()
{
    ...
    if (dlg.exec()) {
        ...
        ui->zMTNSpayAmount->setFocus();
    }
}

void PrivacyDialog::on_pushButtonMintzMTNS_clicked()
{
    if (!walletModel || !walletModel->getOptionsModel())
        return;
    ...
    if(GetAdjustedTime() > GetSporkValue(SPORK_16_ZEROCOIN_MAINTENANCE_MODE)) {
        QMessageBox::information(this, tr("Mint Zerocoin"),
                                 tr("zMTNS is currently undergoing maintenance."), QMessageBox::Ok,
                                 QMessageBox::Ok);
        return;
    }
    ...
    if (encStatus == walletModel->Locked) {
        WalletModel::UnlockContext ctx(walletModel->requestUnlock(AskPassphraseDialog::Context::Mint_zMTNS, true));
        if (!ctx.isValid()) {
    ...
    ui->TEMintStatus->setPlainText(tr("Minting ") + ui->labelMintAmountValue->text() + " zMTNS...");
    ...
    QString strStatsHeader = tr("Successfully minted ") + ui->labelMintAmountValue->text() + tr(" zMTNS in ") +
...
void PrivacyDialog::on_pushButtonSpendzMTNS_clicked()
{
    ...
    if(GetAdjustedTime() > GetSporkValue(SPORK_16_ZEROCOIN_MAINTENANCE_MODE)) {
        QMessageBox::information(this, tr("Mint Zerocoin"),
                                 tr("zMTNS is currently undergoing maintenance."), QMessageBox::Ok, QMessageBox::Ok);
        return;
    }
    ...
    if (encStatus == walletModel->Locked || encStatus == walletModel->UnlockedForAnonymizationOnly) {
        ...
        // Wallet is unlocked now, sedn zMTNS
        sendzMTNS();
        return;
    }
    // Wallet already unlocked or not encrypted at all, send zMTNS
    sendzMTNS();
}

void PrivacyDialog::on_pushButtonZOmoControl_clicked()
{
    ZOmoControlDialog* zOmoControl = new ZOmoControlDialog(this);
    zOmoControl->setModel(walletModel);
    zOmoControl->exec();
}

void PrivacyDialog::setZOmoControlLabels(int64_t nAmount, int nQuantity)
{
    ui->labelzOmoSelected_int->setText(QString::number(nAmount));
...
void PrivacyDialog::sendzMTNS()
{
    ...
    if(ui->payTo->text().isEmpty()){
        ...
        if (!address.IsValid()) {
            QMessageBox::warning(this, tr("Spend Zerocoin"), tr("Invalid CHIPO Address"), QMessageBox::Ok, QMessageBox::Ok);
            ui->payTo->setFocus();
            return;
        }
    }
    ...
    double dAmount = ui->zMTNSpayAmount->text().toDouble();
    ...
    // Warn for additional fees if amount is not an integer and change as zMTNS is requested
    ...
    if(!fWholeNumber && fMintChange){
        ...
        strFeeWarning += QString::number(dzFee, 'f', 8) + " MTNS </b>will be added to the standard transaction fees!<br />";
    ...
    QString strAmount = "<b>" + QString::number(dAmount, 'f', 8) + " zMTNS</b>";
    ...
    // use mints from zOmo selector if applicable
    vector<CZerocoinMint> vMintsSelected;
    if (!ZOmoControlDialog::listSelectedMints.empty()) {
        vMintsSelected = ZOmoControlDialog::GetSelectedMints();
    }
    ...
    if (walletModel && walletModel->getAddressTableModel()) {
        // If zOmo was spent successfully update the addressbook with the label
    ...
    // Clear zomo selector in case it was used
    ZOmoControlDialog::listSelectedMints.clear();
    ui->labelzOmoSelected_int->setText(QString("0"));
    ...
    for (CZerocoinSpend spend : receipt.GetSpends()) {
        strStats += tr("zOmo Spend #: ") + QString::number(nCount) + ", ";
                               ...
                               const CAmount& watchOnlyBalance, const CAmount& watchUnconfBalance, const CAmount& watchImmatureBalance)
{
    ...
    for (const auto& denom : libzerocoin::zerocoinDenomList) {
        ...
        strDenomStats = strUnconfirmed + QString::number(mapDenomBalances.at(denom)) + " x " +
                        ...
                        QString::number(nSumPerCoin) + " zMTNS </b>";
    ...
    ui->labelzAvailableAmount->setText(QString::number(zerocoinBalance/COIN) + QString(" zMTNS "));
    ui->labelzAvailableAmount_2->setText(QString::number(matureZerocoinBalance/COIN) + QString(" zMTNS "));
    ui->labelzMTNSAmountValue->setText(BitcoinUnits::floorHtmlWithUnit(nDisplayUnit, balance - immatureBalance - nLockedBalance, false, BitcoinUnits::separatorAlways));

    // Display AutoMint status
    updateAutomintStatus();

    // Display global supply
    ui->labelZsupplyAmount->setText(QString::number(chainActive.Tip()->GetZerocoinSupply()/COIN) + QString(" <b>zMTNS </b> "));
    for (auto denom : libzerocoin::zerocoinDenomList) {
        ...
        QString strSupply = QString::number(nSupply) + " x " + QString::number(denom) + " = <b>" +
                            QString::number(nSupply*denom) + " zMTNS </b> ";
        switch (denom) {
            case libzerocoin::CoinDenomination::ZQ_ONE:
...
{
    ui->labelzMTNSSyncStatus->setVisible(fShow);
}
