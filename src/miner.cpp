...
CBlockTemplate* CreateNewBlock(const CScript& scriptPubKeyIn, CWallet* pwallet, bool fProofOfStake)
{
    ...
    if (fZerocoinActive)
        ...
        pblock->nVersion = Params().CurrentHeaderVersion();
...
bool ProcessBlockFound(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != chainActive.Tip()->GetBlockHash())
            return error("CHIPOMiner : generated block is stale");
    }
    ...
    if (!ProcessNewBlock(state, NULL, pblock))
        return error("CHIPOMiner : ProcessNewBlock, block not accepted");
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    CValidationState state;
    if (!ProcessNewBlock(state, NULL, pblock))
        return error("CHIPOMiner : ProcessBlock, block not accepted");
...
{
    LogPrintf("CHIPOMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("chipo-miner");
    ...
    while (fGenerateBitcoins || fProofOfStake) {
        if (fProofOfStake) {
            int curretOffset = GetBlockOffset(chainActive.Tip()->nHeight + 1);
            if (curretOffset <= Params().LAST_POW_BLOCK_OFFSET() ) {
                //nLastCoinStakeSearchInterval = 0; kaniTODO
                MilliSleep(5000);
                continue;
            }
        ...
        LogPrintf("Running CHIPOMiner with %u transactions in block (%u bytes)\n", pblock->vtx.size(),
