...
// Copyright (c) 2018 The CHIPO developers
...
bool IsBlockPayeeValid(const CBlock& block, int nBlockHeight)
{
    ...
    int curretOffset = GetBlockOffset(nBlockHeight);
    const CTransaction& txNew = (curretOffset > Params().LAST_POW_BLOCK_OFFSET() ? block.vtx[1] : block.vtx[0]);
...
void CMasternodePayments::FillBlockPayee(CMutableTransaction& txNew, int64_t nFees, bool fProofOfStake)
{
    ...
    CAmount masternodePayment = GetMasternodePayment(pindexPrev->nHeight, blockValue); // todoKANI
...
bool CMasternodeBlockPayees::IsTransactionValid(const CTransaction& txNew)
{
    ...
    if (IsSporkActive(SPORK_8_MASTERNODE_PAYMENT_ENFORCEMENT)) {
        ...
        nMasternodeNum = mnodeman.stable_size() + Params().MasternodeCountDrift();
    }
    else {
        ...
        nMasternodeNum = mnodeman.size() + Params().MasternodeCountDrift();
    }

    CAmount requiredMasternodePayment = GetMasternodePayment(nBlockHeight, nReward, nMasternodeNum);

    //require at least 6 signatures
    BOOST_FOREACH (CMasternodePayee& payee, vecPayments)
        if (payee.nVotes >= nMaxSignatures && payee.nVotes >= MTNSPAYMENTS_SIGNATURES_REQUIRED)
            nMaxSignatures = payee.nVotes;

    // if we don't have at least 6 signatures on a payee, approve whichever is the longest chain
    if (nMaxSignatures < MTNSPAYMENTS_SIGNATURES_REQUIRED) return true;
    ...
    BOOST_FOREACH (CMasternodePayee& payee, vecPayments) {
        ...
        if (payee.nVotes >= MTNSPAYMENTS_SIGNATURES_REQUIRED) {
...
bool CMasternodePaymentWinner::IsValid(CNode* pnode, std::string& strError)
{
    ...
    if (n > MTNSPAYMENTS_SIGNATURES_TOTAL) {
        //It's common to have masternodes mistakenly think they are in the top 10
        // We don't want to print all of these messages, or punish them unless they're way off
        if (n > MTNSPAYMENTS_SIGNATURES_TOTAL * 2) {
            strError = strprintf("Masternode not in the top %d (%d)", MTNSPAYMENTS_SIGNATURES_TOTAL * 2, n);
            LogPrint("masternode","CMasternodePaymentWinner::IsValid - %s\n", strError);
...
bool CMasternodePayments::ProcessBlock(int nBlockHeight)
{
    ...
    if (n > MTNSPAYMENTS_SIGNATURES_TOTAL) {
        LogPrint("mnpayments", "CMasternodePayments::ProcessBlock - Masternode not in the top %d (%d)\n", MTNSPAYMENTS_SIGNATURES_TOTAL, n);
        return false;
    }
