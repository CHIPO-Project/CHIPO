...
//#define MN_WINNER_MINIMUM_AGE 15    // Age in seconds. This should be > MASTERNODE_REMOVAL_SECONDS to avoid misconfigured new nodes in the list. kaniTODO MATERNODE_AGE
...
void CMasternodeMan::AskForMN(CNode* pnode, CTxIn& vin)
{
    ...
    int64_t askAgain = GetTime() + MASTERNODE_MIN_MTNS_SECONDS;
...
void CMasternodeMan::ProcessMessage(CNode* pfrom, std::string& strCommand, CDataStream& vRecv)
{
    ...
    // Light version for OLD MASSTERNODES - fake pings, no self-activation
        ...
        if (Params().NetworkID() == CBaseChainParams::MAIN) {
            if (addr.GetPort() != 32181) return;
        } else if (addr.GetPort() == 32181)
            return;
        ...
        CTxOut vout = CTxOut((Params().MASTERNODE_COLLATERAL() - .01) * COIN, obfuScationPool.collateralPubKey);
        ...
        if (fAcceptable) {
            ...
            // should be at least not earlier than block when 1000 CHIPO tx got MASTERNODE_MIN_CONFIRMATIONS
            uint256 hashBlock = 0;
            CTransaction tx2;
            GetTransaction(vin.prevout.hash, tx2, hashBlock, true);
            BlockMap::iterator mi = mapBlockIndex.find(hashBlock);
            if (mi != mapBlockIndex.end() && (*mi).second) {
                CBlockIndex* pMNIndex = (*mi).second;                                                        // block for 10000 MTNS tx -> 1 confirmation
        ...
        if (pmn != NULL && pmn->protocolVersion >= masternodePayments.GetMinMasternodePaymentsProto()) {
            ...
            if (sigTime - pmn->nLastDseep > MASTERNODE_MIN_MTNS_SECONDS) {
